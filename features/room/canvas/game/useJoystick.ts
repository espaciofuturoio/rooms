...
export const useJoystick = (handleMove: (dx: number, dy: number, direction: PlayerLayout["direction"]) => void) => {
    const lastCallTimeRef = useRef<number>(0);
  
    const throttledHandleMove = useCallback((dx: number, dy: number, direction: PlayerLayout["direction"]) => {
      const now = Date.now();
